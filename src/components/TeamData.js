...
const TeamData = props =>  {
    ...
    const submitVerification = async () => {
        setLoading(true)
        console.log(players);
        const playersToAdd = players.map(p => ({name:p.name, verified: p.verified}));
        const returnedTeam = await updatePlayers(row[0], playersToAdd);
        const formatedReturnedPlayewrs = returnedTeam?.players?.map(p => ({name:p.name, verified: p.verified}));
    ...
    <div style={styles.centerDiv}>
        ...
        <div style={styles.sectionDivs}>
            ...
            <div> <a href={'tel:' +rowDataFromBe.captainPhone}> {rowDataFromBe.captainPhone} </a></div>
            <div> <a href={'mailto:' +rowDataFromBe.captianEmail}> {rowDataFromBe.captianEmail} </a></div>
        </div>

        <div style={styles.sectionDivs}>
            <div>Manager Details</div>
            <div>{rowDataFromBe.managerName}</div>
            <div> <a href={'tel:' +rowDataFromBe.managerPhone}> {rowDataFromBe.managerPhone} </a></div>
            <div> <a href={'mailto:' +rowDataFromBe.managerEmail}> {rowDataFromBe.managerEmail} </a></div>
        </div>
        ...
        <div style={styles.sectionDivs}>
            <button onClick={submitVerification}>Submit Verification <HashLoader cssOverride={override} loading={loading} color="#ffffff" /></button>
        </div>
