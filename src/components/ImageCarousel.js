        ...
        {images.length > 0 ? (
          images.map((image, index) => (
            <Carousel.Item key={index}>
              <img className="d-block w-100 rounded-carousel-img" src={image} alt={`Slide ${index + 1}`} style={{ height: '400px', objectFit: 'cover' }} />
              <Carousel.Caption
                style={{
          ...
          <p style={{ color: '#FFF', textAlign: 'center' }}>Loading images...</p>
        )}
      </Carousel>
    </div>
  );
